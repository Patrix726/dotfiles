...
zellij_switch() {
    zellij pipe --plugin "$HOME/.config/zellij/plugins/zellij-switch.wasm" "$@"
}
...
if [[ -n $SESS ]]; then
    ...
    if [[ -n "$ZELLIJ" ]]; then
        if is_existing_session "$SESS"; then
            ...
            layout=$(get_layout_from_sesh "$SESS")
            layout=${layout:-$LAYOUT}
            record_session_switch "$SESS"
            zellij_switch -- "--session $(printf %q "$SESS") --layout $layout"
        fi
        ...
        if is_directory "$SESS"; then
            ...
            layout=$(get_layout_from_sesh "$session_name")
            layout=${layout:-$LAYOUT}
            zellij --layout "$layout" attach --create "$SESS"
        fi
    fi
fi
